...
def load_service_account():
    """サービスアカウント認証情報を読み込む"""
    secrets_path = Path.home() / ".secrets/ga-service-account.json"
...
def get_search_console_data(credentials, site_url="sc-domain:chirumaru.jp"):
    ...
    try:
        ...
        # 一昨日のデータ
        request_day_before = {
            'startDate': str(day_before_yesterday),
            'endDate': str(day_before_yesterday),
            'dimensions': ['date'],
            'rowLimit': 10000,
        }

        response_day_before = service.searchanalytics().query(
            siteUrl=site_url,
            body=request_day_before
        ).execute()
        ...
        day_before_data = calculate_totals(response_day_before)
...
def check_significant_changes(ga4_data, sc_data, threshold=30):
    ...
    # Search Console の変化チェック
    if "error" not in sc_data and sc_data.get("yesterday") and sc_data.get("day_before"):
        for metric in ["clicks", "impressions"]:
            yesterday_val = sc_data["yesterday"].get(metric, 0)
            day_before_val = sc_data["day_before"].get(metric, 0)
...
def save_data(ga4_data, sc_data, changes):
    """データを JSON に保存"""
    output_file = Path.home() / "Downloads/chirumaru-analytics-daily.json"
...
def print_report(ga4_data, sc_data, changes):
    ...
    if changes:
        ...
        for change in changes:
            ...
            print(f"  前日: {change['yesterday']}")
            print(f"  一昨日: {change['day_before']}")
