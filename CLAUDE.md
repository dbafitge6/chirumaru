...
## 店舗情報の検索・Airtable投入（重要：具体的な依頼方法）

**目的**: 新潟県内のカフェ・パン屋・スイーツ店をリサーチして Airtable に自動投入

### 依頼方法（ユーザー側）

**文型 1 — 単一店舗の調査**:
```
「[店舗名] を Airtable に追加して」
例）「ちるまる Coffee Roasters を Airtable に追加して」
```

**文型 2 — 複数店舗・テーマリサーチ**:
```
「[テーマ]の店舗を[件数]件 Airtable に追加して」
例）「新潟駅周辺のカフェを3件 Airtable に追加して」
例）「五十嵐浜エリアのパン屋を5件調査して追加して」
```

**文型 3 — SNS/ネット情報ソースの指定**:
```
「Instagram/Twitter/ブログで話題の[カテゴリ]を[件数]件追加して」
例）「Instagram で話題のスイーツ店を3件追加して」
```

### エージェント実行フロー

1. ユーザーが上記形式で依頼
2. **store-researcher サブエージェント** が自動起動
   - Web検索/Instagram/ブログを調査
   - 既存レコードと重複チェック（店舗名・住所で一致確認）
   - 重複なし → 新規作成 / 重複あり → 更新
3. Airtable に投入 + 反映確認
4. 投入結果を報告

### フィールド入力ルール（store-researcher が自動適用）

| フィールド | 形式・ルール | 例 |
|----------|----------|-----|
| **店舗名** | 正式な店舗名 | ちるまる Coffee Roasters |
| **住所** | 番地まで含める | 新潟市中央区東大通2-1-1 |
| **エリア** | 区名のみ（重複排除） | 新潟市中央区 |
| **タグ** | "/"区切り、絶対に空欄にしない | カフェ/古民家/駐車場 |
| **メニュー** | フード/ドリンク分け、金額なし | フード：パスタ、クロワッサン / ドリンク：コーヒー |
| **営業時間** | 定休日を含めて1文字列 | 11:00-18:00 月・火定休 |
| **Website** | https:// から始まる形式 | https://example.com（複数行OK） |

### 注意事項

- ❌ **対象外**: ラーメン、居酒屋、重い食事系（設定ルールによる自動除外）
- ⚠️ **確認必須**: 投入後、Airtable で実際に反映されているか自分で確認
- 🔄 **重複処理**: 既存店舗なら「更新」として新情報を反映
