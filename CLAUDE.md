...
### 一言メモ（店舗説明文）
- **文字数**: 35字以内（2～4ページ目スライド）
- **内容**: 店舗の特徴・おすすめを簡潔に表現
- **禁止事項**:
  - 評価語を使わない（旨い、絶品、人気、おすすめ等）
  - 「です」「ます」で終わらない
- **語尾分散**: 体言止め、「〜が名物」「〜特徴」など、バリエーションをつける
- **例**:
  - ✅ 「直火焙煎コーヒーが特徴。温かみのある内装」（18字）
  - ✅ 「シェアスペース内のカフェ」（11字）
  - ❌ 「本当に美味しいコーヒーがおすすめです」（評価語・です止め）

## GitHub への push
- **origin は `/Users/kobayashikazuya/chirumaru-mirror` ローカルミラーフォルダ（GitHub ではない）**
- **GitHub への push は必ず `git push github main` を使う**
- origin ではなく github を指定すること（そうしないと GitHub Actions に反映されない）
